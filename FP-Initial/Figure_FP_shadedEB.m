...
for plotId = 1 : Batch_data{1, 1}.total_freq
     ...
     plot(Batch_data{1, 1}.sorted_data_C (:,:, plotId)', 'color',[201/255, 229/255, 214/255, 0.5], 'LineWidth', 1.5);
     alpha(0.2)
     xticks(1:sampling_rate-1:max(input.data_window)*sampling_rate)
     xticklabels((min(input.data_window):max(input.data_window)-1))
     hold on
     plot(mean(Batch_data{1, 1}.sorted_data_C (:,:, plotId)), 'color',[55/255, 159/255, 69/255], 'LineWidth', 2.5);
     hold off
     ax = gca;
     ax.FontSize = 18;
...
for plotId = 1 : Batch_data{1, 1}.total_freq
     ...
     subplot(3, 3, mod(plotId-1, 9)+1) ;
     shadedErrorBar(frames, Batch_data{1, 1}.mean_data_C(plotId,:), Batch_data{1, 1}.SEM_C(plotId,:), {'markerfacecolor', [186/256, 85/256, 211/256]});
     %ylim([-0.5 1])
     xticks(1:sampling_rate-1:max(input.data_window)*sampling_rate)
     xticklabels((min(input.data_window):max(input.data_window)-1))
